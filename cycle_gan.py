...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters + 1):
        ...
        m3 = D_X(images_X)-torch.ones(len(images_X))
        D_X_loss = torch.mean(m3*m3,dim=0)
        m4 = D_Y(images_Y)-torch.ones(len(images_Y))
        D_Y_loss = torch.mean(m4*m4,dim=0)
        ...
        m5 = D_X(fake_X)
        D_X_loss = torch.mean(m5*m5,dim=0)
        ...
        m6 = D_Y(fake_Y)
        D_Y_loss = torch.mean(m6*m6,dim=0)
        ...
        m7 = D_X(fake_X)-torch.ones(len(fake_X))
        #g_loss = m7*m7
        g_loss = loss(D_X(fake_X),torch.ones(len(fake_X)))
        ...
        if opts.use_cycle_consistency_loss:
            ...
            #m8 = images_Y-reconstructed_Y

            cycle_consistency_loss = loss(images_Y, reconstructed_Y)
            # cycle_consistency_loss = cycle_consistency_loss.unsqueeze(0)
            # print("cyc loss",cycle_consistency_loss.shape)
            g_loss =  g_loss + cycle_consistency_loss
            print(g_loss.shape)
        ...
        m9 = D_Y(fake_Y)-torch.ones(len(fake_Y))
        g_loss = loss(D_Y(fake_Y),torch.ones(len(fake_Y)))

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            m10 = images_X-reconstructed_X
            cycle_consistency_loss = torch.mean(m10*m10)
            g_loss = g_loss + cycle_consistency_loss
