...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            #D_real_loss = (1/2*len(real_images))*torch.sum(torch.mm((D(real_images)-torch.ones(len(real_images))),(D(real_images)-torch.ones(len(real_images)))))
            m1 = D(real_images)-torch.ones(len(real_images))
            D_real_loss = 0.5*torch.mean(m1*m1,dim=0)
            ...
            m2 = D(fake_images)
            D_fake_loss = 0.5*torch.mean(m2*m2,dim=0)
            ...
            m3 = D(fake_images)-torch.ones(len(fake_images))
            G_loss = torch.mean(m3*m3,dim=0)
